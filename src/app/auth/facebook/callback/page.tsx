...
function FacebookCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      requestAnimationFrame(() => setError('ไม่ได้รับ authorization code จาก Facebook'));
      return;
    }

    const redirectUri = `${window.location.origin}/auth/facebook/callback`;

    loginWithSocial('facebook', code, redirectUri).then((result) => {
      if (result.success) {
        router.push('/member');
      } else {
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center">
      ...
      <p className="text-gray-600">กำลังเข้าสู่ระบบด้วย Facebook...</p>
    </div>
  );
}
